...
const double ROTATION_SPEED = 2.0; // Adjust this value to change rotation speed

const double CROSSHAIR_AMPLIFICATION = 3.0; // Adjust this value to change sensitivity
cVector3d lastToolPosition;
...
public:
	DynamicTarget(cWorld* w, double size = 0.2) : world(w), moveInterval(3.0), lastMoveTime(0.0) {
		target = new cShapeSphere(size);
	...
	void moveTarget() {
		double x = -5; // Range: -10 to 10
		double y = (rand() % 2000 - 1000) / 200.0;
		double z = 0; // Range: 0.5 to 10.5
		target->setLocalPos(x, y, z);
	}

	cVector3d getPosition() {
		return target->getLocalPos();
	}
};
...
int main(int argc, char* argv[])
{
	...
	world = new cWorld();
	world->m_backgroundColor.setBlueSky();
	camera = new cCamera(world);
	world->addChild(camera);
	camera->set(cVector3d(12.0, 0.0, 0.0),    // camera position (eye)
	...
	lastToolPosition = tool->getDeviceLocalPos();
...
void updateCameraPosition() {
	...
	if (rotateLeft || rotateRight) {
		...
		cMatrix3d rot;
		rot = camera->getLocalRot();

		// Create rotation matrix
		cMatrix3d rotDelta;
		if (rotateLeft)
			rotDelta.setAxisAngleRotationDeg(cVector3d(0, 0, 1), ROTATION_SPEED);
		else  // rotateRight
			rotDelta.setAxisAngleRotationDeg(cVector3d(0, 0, 1), -ROTATION_SPEED);
		...
		{
			{
				...
				cVector3d currentToolPosition = tool->getDeviceLocalPos();
				cVector3d toolMovement = currentToolPosition - lastToolPosition;

				cVector3d amplifiedMovement = toolMovement * CROSSHAIR_AMPLIFICATION;

				cVector3d newTargetPos = target->getLocalPos() + amplifiedMovement;

				// Clamp the target's movement to a specific range
				/*double clampedX = cClamp(newTargetPos.x(), -10.0, 10.0);
				double clampedY = cClamp(newTargetPos.y(), -10.0, 10.0);
				double clampedZ = cClamp(newTargetPos.z(), 0.5, 10.5);*/

				//newTargetPos.set(clampedX, clampedY, clampedZ);

				target->setLocalPos(newTargetPos);

				lastToolPosition = currentToolPosition;

				// Update bullet trajectory
				if (bulletTraj != nullptr)
				{
					cVector3d weaponPos = tool->getDeviceGlobalPos();
					if (isPistolLoaded) {
						weaponPos.add(cVector3d(0.0, 0, 0.1));
					}
					else if (isRifleLoaded) {
						weaponPos.add(cVector3d(0.0, 0, 0.5));
					}
					bulletTraj->m_pointA = weaponPos;
...
void createBlocks(cWorld* world) {
	const double blockSize = 5.0;  // Increased block size
	const double spacing = 10.0;    // Increased spacing between blocks
	const int numBlocks = 4;       // Reduced number of blocks for a more spread out layout
	...
	for (int i = 0; i < numBlocks; i++) {
		for (int j = 0; j < numBlocks; j++) {
			...
			// Create a larger box
			cCreateBox(block, blockSize, blockSize, blockSize);

			// Position the blocks with more space between them
			double x = i * spacing - (numBlocks - 1) * spacing / 2.0;
			double y = j * spacing - (numBlocks - 1) * spacing / 2.0;
			block->setLocalPos(x, y, 0.0);  // Blocks are now on the ground
...
bool checkCollision(const cVector3d& position) {
	const double blockSize = 5.0;  // Make sure this matches the size in createBlocks
	for (const auto& block : blocks) {
		cVector3d blockPos = block->getLocalPos();

		// Check if position is within the block's bounds
		if (position.x() >= blockPos.x() - blockSize / 2 && position.x() <= blockPos.x() + blockSize / 2 &&
			position.y() >= blockPos.y() - blockSize / 2 && position.y() <= blockPos.y() + blockSize / 2 &&
			position.z() >= blockPos.z() && position.z() <= blockPos.z() + blockSize) {
...
void updateWeaponPositionAndOrientation(cGenericHapticDevicePtr hapticDevice, cToolCursor* tool) {
	...
	cVector3d weaponOffset = cVector3d(-2.0, 0, 0);  
